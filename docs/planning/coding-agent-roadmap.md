...
The whole coding vertical (`coding`, `coding_plan`, `security_auditor`, `plugins/tools/workspace/`,
`apps/backend/infrastructure/codebase/`, workspace/project-runs/GitHub/codebase HTTP routers) was deleted in
commit `0a1b036` and **restored in full**. If you plan another removal, note that the frontend, scheduler
execution targets, `project_runs_runner`, RAG cache invalidation and the `general` agent prompt all reference
it — a partial removal leaves dangling imports rather than a clean cut.

## Conversation goal & todos (2026-09-07)

Coding shares **one** goal/todo layer with every other agent — see `agent-runtime-ux-goal-todos-plan.md`.
`coding` / `coding_plan` have all seven tools (`goal_get|create|update`, `todo_write|read`,
`plan_mode_set`, `exit_plan_mode`); `security_auditor` has goal + todos only. The old
`plugins/tools/workspace/planning/todo.py` is deleted — it kept one module-global list for the whole
process, so every user and conversation shared it and nothing showed up in the UI.

State lives per conversation in `chat_conversations.session_goal / session_todos / session_plan_mode`.
`conversation_goal_prompt_block` injects the usage rules **and** the current goal/todo state into every
turn, so the model does not need a `todo_read` round to see its own list. Goal bar, todo panel and
`agent.goal_round` auto-continue are keyed on `conversation_id`, not on agent id — coding gets them
for free. Extending coding means using these tools, not adding a parallel todo list.
...
| **LSP / Diags** | Echter Code-Intellekt | **Habt ihr:** `lsp` + model-facing diagnostics JSON + [`docs/runbooks/lsp.md`](../runbooks/lsp.md) / `scripts/lsp_path_smoke.py`. **Next:** optional servers in image if ops want zero host install. |
...
- **LSP runbook + diagnostics polish:** `docs/runbooks/lsp.md`, `scripts/lsp_path_smoke.py` (Python/TS PATH), model-facing diagnostics (`summary`, 1-based locations, severity labels), `AGENT_LSP_<LANG>_CMD`.

**Still open:** Index-on-attach progress in UI (flag exists); optional Ripgrep image packaging if `rg` missing.
