...
Output: ``$AGENT_DATA_DIR/output/<user-id>/creative/iterative-html/<timestamp>/``.

**Start from chat:** tool ``build`` (see ``plugins/tools/capabilities/creative/run_iterative_html_build.py``)
...
# Fallback only when ``arguments["goal"]`` is absent — not used by ``build`` from chat.
...
def _output_dir(user_id: int | str | None = None) -> Path:
    from apps.backend.domain.identity import get_identity
    from apps.backend.domain.user_output import user_output_subdir

    if user_id is None:
        _, user_id = get_identity()
    return user_output_subdir(user_id, "creative", "iterative-html")
