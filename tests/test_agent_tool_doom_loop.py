...
class AgentToolDoomLoopTickTests(unittest.TestCase):
    ...
    def test_fires_on_third_repeat_for_non_excluded_tool(self) -> None:
        """Mutating / risky tools still trigger doom (use ``coding_bash``; reads are excluded by default)."""
        args = {"command": "pwd"}
        ex = frozenset()
        k, n, h = _agent_tool_doom_loop_tick(
            None, 0, tool_name="coding_bash", args=args, max_streak=3, exclude_names=ex
        )
        self.assertIsNone(h)
        self.assertEqual(n, 1)
        k, n, h = _agent_tool_doom_loop_tick(k, n, tool_name="coding_bash", args=args, max_streak=3, exclude_names=ex)
        self.assertIsNone(h)
    ...
    def test_excluded_read_tools_never_advance_doom(self) -> None:
        ex = frozenset({"coding_read_file"})
        args = {"path": "README.md"}
        k, n, h = _agent_tool_doom_loop_tick(
            None, 0, tool_name="coding_read_file", args=args, max_streak=3, exclude_names=ex
        )
        self.assertIsNone(h)
        self.assertEqual(n, 0)
        self.assertIsNone(k)
        k2, n2, h2 = _agent_tool_doom_loop_tick(
            k, n, tool_name="coding_read_file", args=args, max_streak=3, exclude_names=ex
        )
        self.assertIsNone(h2)
