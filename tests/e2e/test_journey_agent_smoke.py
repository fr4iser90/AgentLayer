"""Journey C — run-card persistence and WebSocket agent smoke (live LLM)."""
...
from tests.e2e.support.chat_ws import first_catalog_model
...
def test_websocket_agent_tool_smoke(admin_client: E2EClient) -> None:
    ...
    with temporary_self_editing_enabled(admin_client):
        ...
        catalog_model = first_catalog_model(admin_client)
        ...
        events = asyncio.run(_collect_ws_events(admin_client.token, chat_body, timeout_s=180.0))
