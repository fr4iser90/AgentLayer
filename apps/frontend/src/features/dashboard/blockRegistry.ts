...
export const GRID_BLOCK_DEFINITIONS: GridBlockDefinition[] = [
  ...
  gridDef({
    type: "formula_calc",
    dataPathPrefix: "formula",
    addLabelKey: "dashboard:addFormulaCalc",
    defaultGrid: { w: 6, h: 8 },
    minGrid: { minW: 3, minH: 5 },
    mobileStack: true,
    shellHeight: "tall",
    supportsExpand: true,
    expandTitleFallbackKey: "dashboard:formulaCalcFallback",
    createProps: () => ({
      title: "",
      disclaimer: "",
      formulaNote: "",
      formulaInputs: [{ key: "a", label: "a" }, { key: "b", label: "b" }],
      formulaOutputs: [{ key: "sum", label: "a+b", expr: "a+b" }],
    }),
    createInitialData: () => ({}),
  }),
  gridDef({
