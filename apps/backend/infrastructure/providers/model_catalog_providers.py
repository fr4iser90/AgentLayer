...
from apps.backend.infrastructure.agent_runtime.llm_env_providers import (
    EnvLlmProviderRow,
...
def provider_request_headers(spec: CatalogProviderSpec) -> dict[str, str]:
    from apps.backend.infrastructure.settings.operator_settings import external_api_headers
...
def _parse_models_payload(data: dict[str, Any], owned_by: str) -> list[dict[str, Any]]:
    from apps.backend.infrastructure.agent_runtime.context_budget import extract_context_length_from_model_item
...
def fetch_full_model_catalog_for_scope(
    *,
    ...
    try:
        from apps.backend.infrastructure.providers.embedding_client import embedding_catalog_health
    ...
    if tenant_id is not None and user_id is not None:
        from apps.backend.infrastructure.providers.model_access_policy import filter_catalog_rows_for_user
...
from apps.backend.infrastructure.providers.model_catalog_chat_route import (
