...
class _ProviderGate:
    ...
    def _ordered_waiters(self) -> list[_Waiter]:
        from apps.backend.infrastructure.llm_queue_policy import load_queue_config
...
def _acquire_with_policy(provider_key: str, max_parallel: int) -> None:
    from apps.backend.infrastructure.llm_queue_policy import resolve_waiter_meta
