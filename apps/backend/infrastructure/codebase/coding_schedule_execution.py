...
from apps.backend.infrastructure.codebase.coding_workflow import workflow_from_row
...
def _pick_schedule_catalog_provider() -> str | None:
    ...
    from apps.backend.infrastructure.providers.model_catalog_providers import fetch_full_model_catalog
    from apps.backend.infrastructure.settings.operator_settings import normalize_model_catalog_owned_by
...
def _workspace_disk_path(ws_id: uuid.UUID, user_id: uuid.UUID) -> Path | None:
    from apps.backend.domain.workspace.resolver import resolve_workspace
...
async def run_coding_schedule_row(
    ...
    *,
    ...
    """
    from apps.backend.infrastructure.scheduling import scheduler_job_runs_store
    ...
    from apps.backend.domain.scheduling.targets import is_agent_schedulable, normalize_execution_target
    ...
    from apps.backend.domain.shared.identity import llm_queue_source_scope, reset_identity, set_identity
    ...
    if root is not None:
        try:
            from apps.backend.infrastructure.workspace.workspace_git import workspace_git_changes_summary
