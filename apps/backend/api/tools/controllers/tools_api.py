...
from apps.backend.domain.shared.http_identity import resolve_tools_list_identity
...
def _policies_map_safe() -> dict[tuple[str, str], dict[str, Any]]:
    try:
        from apps.backend.application.tools.use_cases.tools_controller_services import policies_map
...
async def admin_list_tools(request: Request):
    ...
    pmap = _policies_map_safe()
    try:
        from apps.backend.application.tools.use_cases.tools_controller_services import list_policies
