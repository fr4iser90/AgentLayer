...
class ModelRoutingSettings:
    ...
    profile_vlm: str | None = None
    profile_agent: str | None = None
    profile_coding: str | None = None
    allow_model_override: bool = True
    override_roles: frozenset[str] = field(default_factory=frozenset)
    override_anonymous: bool = False
...
def _model_for_profile(profile: str, settings: ModelRoutingSettings) -> str:
    """Env profile hints only; chat completions must finalize via ``catalog_chat_llm`` (no env default model id)."""
    if profile == "vlm":
        return (settings.profile_vlm or "").strip()
    if profile == "agent":
        return (settings.profile_agent or "").strip()
    if profile == "coding":
        return (settings.profile_coding or "").strip()
    return (settings.profile_default or "").strip()


def profile_default_model_id(profile: str, settings: ModelRoutingSettings | None = None) -> str:
    """Default model id for ``default`` / ``vlm`` / ``agent`` / ``coding`` (env ``AGENT_MODEL_*``)."""
    return _model_for_profile(_normalize_profile(profile), settings or ModelRoutingSettings())
...
def _override_allowed(bearer_user_role: str | None, settings: ModelRoutingSettings) -> bool:
    if not settings.allow_model_override:
        return False
    if bearer_user_role is None:
        return bool(settings.override_anonymous)
    allowed_roles = settings.override_roles
...
def resolve_effective_model(
    *,
    ...
    routing_settings = settings or ModelRoutingSettings()
    auto_vlm = messages_contain_image_parts(messages)
    if auto_vlm:
        profile = "vlm"
        base = _model_for_profile("vlm", routing_settings)
        ...
        if hdr:
            ...
            profile = "default"
        base = _model_for_profile(profile, routing_settings)
    ...
    if _override_allowed(bearer_user_role, routing_settings) and not auto_vlm:
    ...
    if auto_vlm and _override_allowed(bearer_user_role, routing_settings):
        oh = _strip_model(override_header)
        bm = _strip_model(body_model)
        if _profile_token_from_body(body_model):
            bm = None
