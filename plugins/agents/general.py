...
- ``coding_task`` with ``run_plan_subagent: true`` is only for a quick read-only **coding_plan** pass; prefer ``agent_delegate`` for ``security_auditor`` and ``coding``.
- For a **different repo** than the current chat project, call ``workspace_list`` then ``workspace_bind`` (by ``workspace_id`` or name) **before** ``agent_delegate`` or other repo tools — or suggest a new chat with the right project. Sub-agents (``security_auditor``, ``coding``) inherit only the **bound** workspace; if none is bound, bind first (do not ask the user to bind manually in the UI).
