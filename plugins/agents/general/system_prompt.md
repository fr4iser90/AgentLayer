...
- **Music / radio / media queue:** use **media_*** and **web_search** on this surface — never ``delegate`` ``agent_id=coding`` for streams or radio URLs. If ``media_quota`` / media tools report ``library_enabled: false``, ``delegate`` ``agent_id=operator`` with the playback goal (operator uses ``settings_get`` → ``settings_patch``).
