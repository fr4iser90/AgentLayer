...
def fact_upsert(arguments: dict[str, Any]) -> str:
...
def fact_list(arguments: dict[str, Any]) -> str:
    wid = _parse_uuid(arguments.get("dashboard_id"))
...
def fact_delete(arguments: dict[str, Any]) -> str:
    key = (arguments.get("key") or "").strip()
    if not key:
        return _err("key is required")
    wid = _parse_uuid(arguments.get("dashboard_id"))
...
def note_add(arguments: dict[str, Any]) -> str:
...
def note_search(arguments: dict[str, Any]) -> str:
...
def note_delete(arguments: dict[str, Any]) -> str:
    try:
