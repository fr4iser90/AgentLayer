...
from plugins.tools.knowledge.search import search as coding_search
from plugins.tools.knowledge.semantic_search import semantic_search as coding_semantic_search
from plugins.tools.knowledge.lib.retrieval_fusion import build_fused_ranking
...
def _next_steps(bundle: dict[str, Any]) -> list[str]:
    ...
    if isinstance(fused, list) and fused:
        ...
        if isinstance(top, dict):
            ...
            if path and src in ("code_grep", "code_semantic"):
                ...
                steps.append(f"Read `read_file` on {path}:{line} ({label}, source={src}).")
    ...
    if not steps:
        ...
        if isinstance(grep, dict) and grep.get("ok") and grep.get("matches"):
            ...
            if isinstance(m0, dict) and m0.get("path"):
                steps.append(
                    f"Read `read_file` on {m0.get('path')}:{m0.get('line', 1)} for the top grep hit."
                )
    ...
    if not steps:
        steps.append("Narrow the query or run `index` then retry `retrieve_context`.")
    steps.append("Use `lsp` for definitions/refs after you have a file path.")
    return steps
