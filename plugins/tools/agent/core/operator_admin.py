...
def operator_settings_get(arguments: dict[str, Any]) -> str:
...
def operator_settings_patch(arguments: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.exception("operator_settings_patch")
...
def operator_interfaces_get(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
...
def operator_interfaces_put(arguments: dict[str, Any]) -> str:
...
def operator_external_llm_endpoints_get(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
...
def operator_external_llm_endpoints_put(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    tid, uid = g_adm
...
def operator_external_llm_models_list(arguments: dict[str, Any]) -> str:
...
def admin_tenants_list(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
...
def admin_tenant_create(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    ...
    try:
        ...
        logger.exception("admin_tenant_create")
        return _err(http_500_detail(e))
    return _ok({"tenant": row})


def admin_users_list(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
...
def admin_user_create(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    try:
    ...
    try:
        ...
        logger.exception("admin_user_create")
        return _err(http_500_detail(e))
...
def admin_user_patch(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    try:
...
def admin_tools_catalog(arguments: dict[str, Any]) -> str:
...
def admin_tool_policies_put(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    try:
    ...
    try:
        ...
        logger.exception("admin_tool_policies_put")
        return _err(http_500_detail(e))
    return _ok({"count": len(body.policies)})


def admin_reload_tools(arguments: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.exception("admin_reload_tools")
        return _err(http_500_detail(e))
    names = []
...
def admin_rag_ingest(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    ...
    try:
        ...
        logger.exception("admin_rag_ingest")
        return _err(http_500_detail(e))
...
def admin_rag_ingest_docs(arguments: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.exception("admin_rag_ingest_docs")
        return _err(http_500_detail(e))
    return json.dumps({"ok": True, "result": out}, ensure_ascii=False, default=str)
...
def admin_scheduler_job_list(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_job_create(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_job_patch(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_job_set_enabled(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_job_set_archived(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_job_delete(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_scheduler_presets_list(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
...
def admin_project_runs_list(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
def admin_project_run_create(arguments: dict[str, Any]) -> str:
    g_adm = _require_admin()
    if isinstance(g_adm, str):
        return g_adm
    _tid, uid = g_adm
...
HANDLERS: dict[str, Callable[[dict[str, Any]], str]] = {
    "operator_settings_get": operator_settings_get,
    "operator_settings_patch": operator_settings_patch,
    "operator_interfaces_get": operator_interfaces_get,
    "operator_interfaces_put": operator_interfaces_put,
    "operator_external_llm_endpoints_get": operator_external_llm_endpoints_get,
    "operator_external_llm_endpoints_put": operator_external_llm_endpoints_put,
    "operator_external_llm_models_list": operator_external_llm_models_list,
    "admin_tenants_list": admin_tenants_list,
    "admin_tenant_create": admin_tenant_create,
    "admin_users_list": admin_users_list,
    "admin_user_create": admin_user_create,
    "admin_user_patch": admin_user_patch,
    "admin_tools_catalog": admin_tools_catalog,
    "admin_tool_policies_put": admin_tool_policies_put,
    "admin_reload_tools": admin_reload_tools,
    "admin_rag_ingest": admin_rag_ingest,
    "admin_rag_ingest_docs": admin_rag_ingest_docs,
    "admin_scheduler_job_list": admin_scheduler_job_list,
    "admin_scheduler_job_create": admin_scheduler_job_create,
    "admin_scheduler_job_patch": admin_scheduler_job_patch,
    "admin_scheduler_job_set_enabled": admin_scheduler_job_set_enabled,
    "admin_scheduler_job_set_archived": admin_scheduler_job_set_archived,
    "admin_scheduler_job_delete": admin_scheduler_job_delete,
    "admin_scheduler_presets_list": admin_scheduler_presets_list,
    "admin_project_runs_list": admin_project_runs_list,
    "admin_project_run_create": admin_project_run_create,
}
...
TOOLS: list[dict[str, Any]] = [
    ...
    _tool_fn(
        "operator_external_llm_endpoints_get",
        "List external LLM endpoints (api keys redacted).",
        {"type": "object", "properties": {}},
    ),
    _tool_fn(
        "operator_external_llm_endpoints_put",
    ...
    _tool_fn(
        "operator_external_llm_models_list",
    ...
    _tool_fn("admin_tenants_list", "List tenants.", {"type": "object", "properties": {}}),
    _tool_fn(
        "admin_tenant_create",
        "Create tenant; name (string).",
        {"type": "object", "properties": {"name": {"type": "string"}}, "required": ["name"]},
    ),
    _tool_fn("admin_users_list", "List all users.", {"type": "object", "properties": {}}),
    _tool_fn(
        "admin_user_create",
    ...
    _tool_fn("admin_tools_catalog", "Tool metadata + operator policy rows.", {"type": "object", "properties": {}}),
    _tool_fn(
        "admin_tool_policies_put",
    ...
    _tool_fn(
        "admin_scheduler_job_set_enabled",
    ...
    _tool_fn(
        "admin_scheduler_job_set_archived",
