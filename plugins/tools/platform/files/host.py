...
Actual reachability is limited by **OS permissions** and deployment (e.g. container mounts, ``execution_context``) — not by ``AGENT_WORKSPACE_ROOT`` (removed). Separate from ``get_file`` (remote API).
"""
...
def stat(arguments: dict[str, Any]) -> str:
...
def glob(arguments: dict[str, Any]) -> str:
