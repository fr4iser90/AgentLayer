"""Shared SimpleSecCheck (scan.fr4iser.com) HTTP client and helpers for security_scan tools."""
...
SCAN_IN_PROGRESS_GUIDANCE = (
    "Scan still in progress. Use deferred_wait with poll_tool='status', "
    "poll_arguments={'scan_id': '<scan_id>'}, estimated_time_seconds from the scanner response "
    "when present, and wait_label='security_scan' to wait in this run (timer paused). "
    "Or end the run and call status or deferred_wait again in a later session."
)

# Back-compat alias for tools that imported the old name.
END_RUN_GUIDANCE = SCAN_IN_PROGRESS_GUIDANCE

NO_WAIT_SUFFIX = (
    " Returns immediately; use deferred_wait to poll until complete — never block or sleep here."
)
    ...
    *,
    ...
    if r.status_code >= 400:
        ...
        if r.status_code == 409:
            ...
            out["agent_guidance"] = [SCAN_IN_PROGRESS_GUIDANCE]
...
def agent_guidance_for_status(
    ...
    *,
    ...
    if st in ("started", "scanning", "queued", "running", "pending"):
        status_lines = [SCAN_IN_PROGRESS_GUIDANCE]
    elif st == "ready":
        status_lines = [
