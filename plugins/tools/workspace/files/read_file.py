...
# Router phrases: co-located read_file.router.yaml (all locales unioned at load).
...
def read_file(arguments: dict[str, Any], context: dict | None = None) -> str:
        ...
        return json.dumps(
            {
                "ok": False,
                "error": (
                    "coding_read_file requires a non-empty string field \"path\" (file path relative to workspace root). "
                    'Example: {"path": "README.md"} or {"path": "src/app.ts", "start_line": 1, "limit_lines": 80}'
                ),
            },
            ensure_ascii=False,
        )
