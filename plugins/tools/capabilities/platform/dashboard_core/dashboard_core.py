...
def dashboard_list(arguments: dict[str, Any]) -> str:
    del arguments
    ident = _identity()
    if ident is None:
...
def dashboard_read(arguments: dict[str, Any]) -> str:
    ident = _identity()
    if ident is None:
        return _err("No user identity — dashboard tools need an authenticated chat user.")
    tid, uid = ident
...
def dashboard_patch_data(arguments: dict[str, Any]) -> str:
    ident = _identity()
    if ident is None:
        return _err("No user identity — dashboard tools need an authenticated chat user.")
    tid, uid = ident
...
def dashboard_patch_layout(arguments: dict[str, Any]) -> str:
    ident = _identity()
    if ident is None:
        return _err("No user identity — dashboard tools need an authenticated chat user.")
    tid, uid = ident
    ...
    if updated is None:
        return _err("could not update dashboard (viewer or conflict)")
    blocks = new_ul.get("blocks") if isinstance(new_ul.get("blocks"), list) else []
    return json.dumps(
        {
            "ok": True,
            "dashboard_id": str(wid),
...
HANDLERS: dict[str, Callable[[dict[str, Any]], str]] = {
    ...
    "dashboard_patch_data": dashboard_patch_data,
    "dashboard_patch_layout": dashboard_patch_layout,
}
