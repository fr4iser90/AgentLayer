...
def workspace_list(arguments: dict[str, Any], context: dict[str, Any] | None = None) -> str:
...
def workspace_create(arguments: dict[str, Any], context: dict[str, Any] | None = None) -> str:
    user = user_from_context(context)
    if user is None:
        return dump({"ok": False, "error": "not authenticated"})
...
def workspace_bind(arguments: dict[str, Any], context: dict[str, Any] | None = None) -> str:
    user = user_from_context(context)
    if user is None:
        return dump({"ok": False, "error": "not authenticated"})
